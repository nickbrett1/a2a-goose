...
# "a2a-goose-dev" (the repo name plus the "-dev"
...
# than adding a second one - a stale "a2a-goose-dev" is overwritten by
...
PROJECT_NAME="a2a-goose"
AGENT_NAME="a2a-goose-dev"
WORKSPACE="/workspaces/a2a-goose"
...
if [ -x "/workspaces/a2a-goose/scripts/agent-dev.sh" ]; then
  "/workspaces/a2a-goose/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
...
# Some hosts run the deployed agent as a *host process* rather than out of a
# devcontainer - on macOS a launchd job (deploy/launchd), on Synology a DSM Task
# Scheduler boot script (deploy/dsm). Those deployments author ENV_FILE and
# CONFIG_FILE by hand, in shapes this script does not emit (shell `export`
# lines; top-level `executor:`/`tracing:`), and they carry settings no generator
# knows about - A2A_GOOSE_BIND, A2A_GOOSE_PUBLIC_URL, LITELLM_CUSTOM_HEADERS,
# GOOSE_BIN, the hub token. Regenerating over them would silently drop those,
# and a second agent started here would race the one the unit already runs. So
# this script refuses to manage a deployment it did not write.
is_host_deployed() {
...
cmd_start() {
  ...
  if is_host_deployed; then
    loud "The agent was NOT started: ${CONFIG_DIR} is maintained by hand, not by this script." \
      "This host runs the host-process deployment - a launchd job on macOS" \
      "(deploy/launchd) or the DSM boot script on Synology (deploy/dsm) - and" \
      "that unit authors env + config.yaml and starts the agent. Running this" \
      "script would rewrite both, dropping settings it does not know about" \
      "(executor, tracing, A2A_GOOSE_BIND, LITELLM_CUSTOM_HEADERS, ...), and race" \
      "the agent that is already running. Start the unit instead, not this script." \
